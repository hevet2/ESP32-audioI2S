...
int32_t VorbisDecoder::parseVorbisComment(uint8_t* inbuf, int16_t nBytes, uint32_t current_file_pos) {
    ...
    auto parse_comment = [&](ps_ptr<char> comment) -> void {
        ...
        if (key.starts_with_icase("work")) { audio.infof(audio, Audio::evt_id3data, "Work: {}", val.c_get()); }
        if (key.starts_with_icase("composer")) { audio.infof(audio, Audio::evt_id3data, "Composer: {}", val.c_get()); }
        if (key.starts_with_icase("genre")) { audio.infof(audio, Audio::evt_id3data, "Genre: {}", val.c_get()); }
        if (key.starts_with_icase("date")) { audio.infof(audio, Audio::evt_id3data, "Date: {}", val.c_get()); }
        if (key.starts_with_icase("album")) { audio.infof(audio, Audio::evt_id3data, "Album: {}", val.c_get()); }
        if (key.starts_with_icase("comment")) { audio.infof(audio, Audio::evt_id3data, "Comments: {}", val.c_get()); }
        if (key.starts_with_icase("tracknumber")) { audio.infof(audio, Audio::evt_id3data, "Track number/Position in set: {}", val.c_get()); }
...
int32_t VorbisDecoder::vorbis_book_unpack(codebook_t* s) {
    ...
    ret = bitReader(16);
    if (ret < 0) VORBIS_LOG_ERROR("error in vorbis_book_unpack, ret ={}\n", ret);
    if (ret > 255) VORBIS_LOG_ERROR("error in vorbis_book_unpack, ret ={}\n", ret);
    s->dim = (uint8_t)ret;
